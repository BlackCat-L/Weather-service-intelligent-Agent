...
你会收到同事提供的数据结果与规则结果。你的职责：
- 把规则中的阈值与实际数据逐条比对，判断是否达标
...
def _call_agent(name: str, question: str, registry, system_prompt: str,
                context: str = "", on_event=None) -> dict:
    ...
    try:
        result = run_loop(user, registry, prompt, on_event=on_event)
...
def run_multi(question: str, registry, on_event=None, max_workers: int = 2) -> dict:
    """多智能体协同执行。
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        f_data = pool.submit(_call_agent, "数据", question, data_tools,
                             DATA_AGENT_PROMPT, "", on_event)
        f_kb = pool.submit(_call_agent, "知识", question, kb_tools,
                           KNOWLEDGE_AGENT_PROMPT, "", on_event)
    ...
    final_trace: list = []
    try:
        final = run_loop(user, registry.subset([]), SUMMARIZER_PROMPT, on_event=on_event)
